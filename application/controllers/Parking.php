...
class Parking extends Admin_Controller
{
	...
	public function printInvoice($id)
	{
		...
		if ($id) {
			...
			$cgeck_out = date("h:i a", $parking_data['out_time']);
			$total_time = ceil((abs($parking_data['out_time'] - $parking_data['in_time']) / 60) / 60);
			$rate_data = $this->model_rates->getRateData($parking_data['rate_id']);
					...
					<div class="main-content">
						...
						<div class="parking-info">
							<table class="table" border=1>
								<tr>
									<td>Check-in Date: ' . $check_in_date . '</td>
									<td>Check-in Time: ' . $check_in . '</td>
								</tr>
								<tr>
									<td>Check-out Date: ' . $check_out_date . '</td>
									<td>Check-out Time: ' . $cgeck_out . '</td>
								</tr>
								<tr>
									<td>Vehicle type: ' . ucwords($vehicle_category['name']) . ' </td>
								</tr>
								<tr>
									<td>Total Time: ' . $total_time. ' Hour </td>
								</tr>
								<tr>
									<td>Total Amount: $' . $earned_amount . ' </td>
								</tr>
							</table>
